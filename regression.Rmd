...
regression_df = boston_df %>%
  select(seconds, age, country_residence, gender, year, half) %>%
  mutate(half = replace(half, half == "-", "")) %>%  
  drop_na() %>%
  mutate(regression_df[regression_df==""]<-NA %>%
  drop_na() %>%
  mutate(time = hms(half))
...
regression_df = regression_df %>%
    mutate(half_seconds = hour(time)*3600 + minute(time)*60 + second(time))

```

```{r}
colSums(is.na(regression_df))
```

unlist(strsplit("half", "[[:space:]]+"))
